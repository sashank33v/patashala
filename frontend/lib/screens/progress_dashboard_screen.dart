        ...
        child: ListView(
          ...
          children: [
            ...
            if (!_settingsLoaded)
              ...
              const Text('Accessibility', style: TextStyle(color: NeonPalette.subtext)),
              SwitchListTile(
                value: _settings.highContrast,
                onChanged: (v) => _updateSettings(_settings.copyWith(highContrast: v)),
                title: const Text('High Contrast UI'),
              ),
              SwitchListTile(
                value: _settings.reducedMotion,
                onChanged: (v) => _updateSettings(_settings.copyWith(reducedMotion: v)),
              ...
              SwitchListTile(
                ...
                onChanged: (v) => _updateSettings(_settings.copyWith(notifications: v)),
      ...
      body: MeshBackground(
        child: FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snapshot) {
            ...
            final overall = ((((trig['percent'] as num).toDouble()) + ((mens['percent'] as num).toDouble())) / 2) / 100;
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    GlassCard(
                      child: Row(
                        children: [
                          Expanded(child: Text('Trigonometry: ${trig['percent']}%', style: const TextStyle(color: NeonPalette.cyan))),
                          Expanded(child: Text('Mensuration: ${mens['percent']}%', style: const TextStyle(color: NeonPalette.pink))),
                        ],
                      ),
                    ),
                    const SizedBox(height: 12),
                    const Text('Top Learners', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
                    const SizedBox(height: 8),
                    ...leaders.take(5).map((l) => Padding(
                          ...
                          child: GlassCard(
                            child: Row(
                              children: [
                                CircleAvatar(backgroundColor: Colors.white10, child: Text('#${l['rank']}')),
                                const SizedBox(width: 10),
                                Expanded(child: Text(l['name']?.toString() ?? 'Student')),
                                Text('${l['completed_count']} topics', style: const TextStyle(color: NeonPalette.cyan)),
                              ],
                            ),
                          ),
                        )),
